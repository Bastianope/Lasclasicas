  ...
  useEffect(() => {
    ...
    d3.range(120).forEach(() => {
      svg.append('circle')
        ...
        .attr('fill', '#D4A847').attr('opacity', Math.random() * 0.3 + 0.1)
    })
    ...
    d3.json('https://cdn.jsdelivr.net/npm/datamaps@0.5.10/src/js/data/arg.topo.json')
      .then((topo: any) => {
        const features = (topojson.feature(topo, topo.objects.arg) as any).features

        svg.selectAll('path.prov').data(features).join('path')
          .attr('class', 'prov').attr('d', path as any)
          .attr('fill', '#141820')
          .attr('stroke', 'rgba(212,168,71,0.25)')
          .attr('stroke-width', 0.6)
          .style('cursor', 'pointer')
          .on('mouseover', function() {
            d3.select(this as any).attr('fill', '#1e2535')
          })
          .on('mouseout', function() {
            d3.select(this as any).attr('fill', '#141820')
          })
        ...
        cities.forEach((city) => {
          const [cx, cy] = projection([city.lon, city.lat]) as [number, number]
